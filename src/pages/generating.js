...
import { FiCheckCircle, FiLoader, FiCpu, FiFileText, FiImage, FiVideo, FiServer, FiActivity } from "react-icons/fi";
    ...
    useEffect(() => {
        ...
        const runGeneration = async () => {
            try {
                ...
                const errorMsg = err.response?.data?.message || "Generation failed";

                if (errorMsg === 'common.content_policy_violation') {
                    toast.error(t('common.content_policy_violation'));
                } else if (err.response?.status === 503) {
                    toast.error(t('generating.server_busy') || "AI Server is busy. Retrying automatically...");
                } else {
                    toast.error(errorMsg);
                }

                setTimeout(() => navigate('/dashboard/generate-course', { replace: true }), 4000);
            }
        };
