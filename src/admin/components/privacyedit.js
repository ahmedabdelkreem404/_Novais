...
const PrivacyEdit = () => {
    ...
    async function getData() {
        try {
            ...
            const response = await axios.get(serverURL + '/admin/pages/privacy', config);
            if (response.data) {
    ...
    async function savePrivacy() {
        try {
            ...
            const postURL = serverURL + '/admin/pages/privacy';
            ...
            if (response.data) {
                localStorage.setItem('privacy_en', terms);
                localStorage.setItem('privacy_ar', termsAr);
                showToast(t('admin.dashboard.policies.privacy_updated'));
            }
        } catch (error) {
            console.error(error);
    ...
    return (
        <div className="p-4 space-y-4">
            <div className="flex justify-between items-center mb-4">
                <h1 className="text-2xl font-bold dark:text-white">{t('footer.privacy')}</h1>
                <button
                    onClick={savePrivacy}
                    disabled={isSaving}
                ...
                {activeTab === 'en' ? (
                    <div dir="ltr">
                        <ReactQuill
                            ...
                            placeholder={t('admin.dashboard.policies.privacy_placeholder_en')}
                        />
                    </div>
                ) : (
                    <div dir="rtl">
                        <ReactQuill
                            ...
                            formats={formats}
                            className="h-[50vh] sm:h-[65vh] mb-12 dark:text-white"
                            placeholder={t('admin.dashboard.policies.privacy_placeholder_ar')}
                        />
                    </div>
                )}
            </div>
