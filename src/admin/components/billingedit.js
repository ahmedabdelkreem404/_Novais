...
const BillingEdit = () => {
    ...
    async function getData() {
        try {
            ...
            const response = await axios.get(serverURL + '/admin/pages/billing', config);
            if (response.data) {
    ...
    async function saveBilling() {
        try {
            ...
            const postURL = serverURL + '/admin/pages/billing';
            ...
            if (response.data) {
                localStorage.setItem('billing_en', terms);
                localStorage.setItem('billing_ar', termsAr);
                showToast(t('admin.dashboard.policies.billing_updated'));
            }
        } catch (error) {
            console.error(error);
    ...
    return (
        <div className="p-4 space-y-4">
            <div className="flex justify-between items-center mb-4">
                <h1 className="text-2xl font-bold dark:text-white">{t('footer.billing')}</h1>
                <button
                    onClick={saveBilling}
                    disabled={isSaving}
                ...
                {activeTab === 'en' ? (
                    <div dir="ltr">
                        <ReactQuill
                            ...
                            placeholder={t('admin.dashboard.policies.billing_placeholder_en')}
                        />
                    </div>
                ) : (
                    <div dir="rtl">
                        <ReactQuill
                            ...
                            formats={formats}
                            className="h-[50vh] sm:h-[65vh] mb-12 dark:text-white"
                            placeholder={t('admin.dashboard.policies.billing_placeholder_ar')}
                        />
                    </div>
                )}
            </div>
