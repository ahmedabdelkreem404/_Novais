...
const RefundEdit = () => {
    ...
    async function getData() {
        try {
            ...
            const response = await axios.get(serverURL + '/admin/pages/refund', config);
            if (response.data) {
    ...
    async function saveRefund() {
        try {
            ...
            const postURL = serverURL + '/admin/pages/refund';
            ...
            if (response.data) {
                localStorage.setItem('refund_en', terms);
                localStorage.setItem('refund_ar', termsAr);
                showToast(t('admin.dashboard.policies.refund_updated'));
            }
        } catch (error) {
            console.error(error);
    ...
    return (
        <div className="p-4 space-y-4">
            <div className="flex justify-between items-center mb-4">
                <h1 className="text-2xl font-bold dark:text-white">{t('footer.refund')}</h1>
                <button
                    onClick={saveRefund}
                    disabled={isSaving}
                ...
                {activeTab === 'en' ? (
                    <div dir="ltr">
                        <ReactQuill
                            ...
                            placeholder={t('admin.dashboard.policies.refund_placeholder_en')}
                        />
                    </div>
                ) : (
                    <div dir="rtl">
                        <ReactQuill
                            ...
                            formats={formats}
                            className="h-[50vh] sm:h-[65vh] mb-12 dark:text-white"
                            placeholder={t('admin.dashboard.policies.refund_placeholder_ar')}
                        />
                    </div>
                )}
            </div>
